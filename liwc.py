...
def _a(flags: list[str], dest: str, *, is_bool: bool = False, **kw) -> dict:
    """Shorthand to build a catalogue entry."""
    entry = {"flags": flags, "dest": dest, "is_bool": is_bool}
    if is_bool:
...
def build_parser() -> argparse.ArgumentParser:
    """Construct the full argument parser from MODE_DEFS."""
    ...
    if not _is_liwc_running():
        if args.auto_open:
            ...
            sys.exit(
                ...
                "(or the license server) first, or re-run with --auto-open."
            )
